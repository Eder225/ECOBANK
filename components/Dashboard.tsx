...
const Dashboard: React.FC<DashboardProps> = ({ user, accounts, transactions, lang, setActiveTab, addNotification }) => {
  ...
  // Calcul dynamique des statistiques basé sur les transactions réelles
  ...
  // Données du graphique basées sur le solde actuel
  const currentBalance = accounts[0]?.balance || 0;
  const chartData = [
    { name: 'Dec', value: 0 },
    { name: 'Jan', value: 195000000 },
    { name: 'Feb', value: currentBalance },
  ...
  return (
    <div className="space-y-6 pb-10 max-w-full relative" onClick={() => setActiveMenu(null)}>
              ...
              <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
                  ...
                  <form onSubmit={handleCreateGoal} className="space-y-4">
                      <div>
                          ...
                          <input type="text" value={newGoalName} onChange={(e) => setNewGoalName(e.target.value)} className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200" placeholder={lang === Language.FR ? "Ex: Vacances, Voiture..." : "Ex: Holiday, Car..."} />
                      </div>
                      <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{t.targetAmount} (XOF)</label>
                          <input type="number" value={newGoalAmount} onChange={(e) => setNewGoalAmount(e.target.value)} className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200" placeholder="0" />
                      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-6">
        <div className="bg-white rounded-[1.5rem] md:rounded-3xl p-4 md:p-6 shadow-sm border border-slate-50">
        ...
        <div className="hidden lg:block bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <select className="bg-slate-50 border-none text-sm font-semibold text-slate-600 rounded-lg px-3 py-1 outline-none"><option>{t.timeRange}</option></select>
                </div>
            </div>
        ...
        <div className="bg-white rounded-[2rem] md:rounded-3xl p-4 md:p-6 shadow-sm border border-slate-50 md:border-slate-100 flex flex-col">
            ...
            <div className={`relative w-full aspect-[1.586/1] rounded-[2rem] p-6 md:p-9 text-white flex flex-col justify-between shadow-2xl overflow-hidden transition-all duration-300 group ${userCard.status === 'frozen' ? 'bg-slate-500 grayscale' : 'shadow-teal-900/10 md:shadow-teal-900/20'}`} style={{ backgroundColor: userCard.status === 'frozen' ? undefined : '#00A88F' }}>
                ...
                <div className="flex justify-between items-start z-10">
                    <div className="w-10 h-7 md:w-14 md:h-11 bg-gradient-to-tr from-[#d4af37] via-[#f9e58e] to-[#d4af37] rounded-md md:rounded-lg"></div>
                ...
                <div className="z-10 flex justify-between items-end">
                    ...
                    <div className="shrink-0 text-right">
                         <p className="text-[8px] md:text-xs uppercase opacity-75 mb-0.5">{t.expires}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="bg-white rounded-[2rem] md:rounded-3xl p-6 shadow-sm border border-slate-50 flex flex-col h-full">
            ...
            <div className="space-y-4">
                {transactions.map(tx => (
                    <div key={tx.id} className="flex items-center justify-between py-1 md:py-0">
                        <div className="flex items-center gap-3 md:gap-4">
                            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center p-2 ${tx.status === 'failed' ? 'bg-red-50 text-red-500' : 'bg-slate-100 text-slate-500'}`}><img src={tx.logo} alt={tx.recipient} className="w-full h-full object-contain" /></div>
                            <div><p className={`font-bold text-sm ${tx.status === 'failed' ? 'text-red-500' : 'text-slate-900'}`}>{tx.recipient}</p><p className="text-[10px] text-slate-400">{formatDate(tx.date)}</p></div>
                        </div>
                        <p className={`font-bold text-sm ${tx.type === 'credit' ? 'text-green-600' : 'text-slate-900'}`}>{tx.type === 'credit' ? '+' : '-'}{formatCurrency(tx.amount)}</p>
                    </div>
                ))}
            </div>
        </div>
